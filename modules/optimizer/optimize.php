...
//  Web mode: page shell
// ---------------------------------------------------------------------------
$page_title = $RULE_ID ? 'Оптимизация одного правила' : 'Оптимизация истории';
?><!DOCTYPE html>
<html lang="ru">
<head>
<meta charset="utf-8">
...
<div class="card">
  ...
  <div class="phase" id="phase">Запуск…</div>
...
<div class="tiles" id="tiles">
  <div class="tile"><b id="t_props">0</b><span>Свойств обработано</span></div>
  <div class="tile"><b id="t_before">0</b><span>Записей было</span></div>
  <div class="tile"><b id="t_after">0</b><span>Записей стало</span></div>
  <div class="tile ok"><b id="t_removed">0</b><span>Удалено</span></div>
</div>

<div class="card">
  <table>
    <thead><tr>
      <th>Свойство</th><th>Режим</th><th class="n">Было</th><th class="n">Стало</th>
      <th class="n">Удалено</th><th style="width:22%">Сжатие</th>
    </tr></thead>
    <tbody id="rows"><tr id="norows"><td colspan="6" class="empty">Пока ничего не изменено…</td></tr></tbody>
  </table>
  <details>
...
(function () {
  ...
  function log(text) {
    ...
    // прокручивается только сам лог, страница остаётся на месте
  ...
  window.P = function (d) {
    switch (d.e) {
      ...
      case 'rule':
        log('+ создано правило: ' + d.title + ' (' + nf(d.total) + ' записей)');
        break;

      case 'analyzed':
        log('анализ завершён: свойств ' + nf(d.properties) + ', записей ' + nf(d.records) +
            ', новых правил ' + nf(d.rules_added));
        break;

      case 'stage':
        stageNo++;
        if (curItem) { $('cur').textContent = curItem + '  — этап ' + stageNo + '/4'; }
        log('  ' + d.from + ' .. ' + d.to + ' | шаг ' + d.interval + ' c | ' +
      ...
      case 'error':
        ...
        $('final').innerHTML =
          '<div class="card final err"><h2>Ошибка</h2><div id="emsg"></div>' +
          '<div class="hint">Данные могли быть обработаны частично. Подробности — в подробном логе выше.</div></div>';
      ...
      case 'done':
        ...
        $('final').innerHTML =
          '<div class="card final"><h2>Оптимизация завершена</h2>' +
          '<div>Просмотрено свойств: <b>' + nf(d.properties) + '</b>, изменено: <b>' + nf(d.changed) + '</b>.</div>' +
          '<div>Удалено записей: <b>' + nf(d.removed) + '</b>' +
          (totalBefore > 0 ? ' (' + nf(totalBefore) + ' → ' + nf(totalAfter) + ', минус ' + pct + '%)' : '') +
          '.</div>' +
          '<div>Время выполнения: <b>' + $('timer').textContent + '</b>' +
          (secs > 0 ? '' : '') + '.</div>' +
          '<div class="hint">Окно можно закрыть. Список правил обновится после перезагрузки страницы модуля.</div>' +
          '</div>';
        break;
    }
  };
...
<?php
// заполнитель: браузеры начинают отрисовку только получив первый килобайт
...
$optimizer->progress = function ($event, $data) {
    ...
    if ($event === 'phase') {
        // новая фаза - следующее событие прогресса должно пройти без задержки
        $last_scan = 0.0;
    }

    // события сканирования могут идти сотнями в секунду - ограничиваем поток,
    // но первое и последнее в фазе пропускаем всегда, иначе полоса застревает
    if ($event === 'scan') {
