...
class optimizer extends module
{
    ...
    function analyze(&$out, $total_limit = 0, $auto_append = 0)
    {
        ...
        $this->report('phase', array('text' => 'Анализ истории', 'total' => $total));
    ...
    private function removeUnsortedData()
    {
        ...
        $this->report('phase', array('text' => 'Очистка потерянных записей'));
        $this->trace("Поиск потерянных записей истории");
        ...
        $this->trace("Потерянных записей: $total");

        if ($total > 0) {
            SQLExec("DELETE FROM phistory WHERE " . $this->orphanCondition('phistory'));
            $this->trace("Потерянные записи удалены");
    ...
    function optimizeAll($id = 0, $object = '', $property = '')
    {
        ...
        if (!count($rules)) {
            ...
            $this->report('warning', array('text' => 'Не задано ни одного правила оптимизации'));
        }
        ...
        $this->report('phase', array('text' => 'Прореживание истории', 'total' => $total));
        ...
        for ($i = 0; $i < $total; $i++) {
            ...
            $this->trace('Свойство ' . $item_title . ' (правило: ' . $key . ')');
            ...
            if (isset($rule['keep'])) {
                $this->trace("  удаление записей старше " . (int)$rule['keep'] . " дн.");
            ...
            if (!empty($rule['optimize'])) {
                ...
                if (!is_array($tmp) || $tmp['TS'] === null) {
                    $this->trace("  нет данных, пропуск");
                    continue;
                }

                $start = (int)$tmp['TS'];

                $this->trace("  период: старше месяца");
            ...
            $this->trace("  итого: " . $total_before . " -> " . $total_after);
        ...
        if ($keep_cached > 0) {
            ...
            $this->trace("Удалено кэш-файлов: " . $deleted);
        }

        $this->trace("Готово. Всего удалено записей: " . $total_records_removed);
    ...
    function optimizeHistoryData($valueID, $type, $interval, $start, $end)
    {
        ...
        $this->trace("    записей в периоде: " . $totalValues);
        ...
        if ($totalValues <= ($optimalValues + 50)) {
            $this->trace("    уже оптимально (" . $totalValues . " ~ " . $optimalValues . "), пропуск");
            return 0;
        }
        ...
        $this->trace("    удалено: $totalRemoved");
        if ($totalRemoved > 0) {
